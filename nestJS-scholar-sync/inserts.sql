-- ============================================================
-- Seed data para desarrollo — scholar-sync
-- Contraseña de TODOS los usuarios: Password123
-- Hash generado con bcrypt, salt rounds = 10
-- Deja roles y permisos incompletos a propósito para poder
-- probar los endpoints POST desde Postman
-- ============================================================

-- 1) users
--    Todos usan la misma contraseña: Password123
INSERT INTO users (email, password, first_name, last_name, profile_pic, major_1, major_2, xp, level) VALUES
('alice@example.com',   '$2b$10$R97gWO76R5TgXjRhl6jji.kxSRC0K8PhBkLAOfMfskAc1AMZHBz/2', 'Alice', 'Anderson', NULL, 'Software Engineering', NULL,        10, 1),
('bob@example.com',     '$2b$10$R97gWO76R5TgXjRhl6jji.kxSRC0K8PhBkLAOfMfskAc1AMZHBz/2', 'Bob',   'Brown',    NULL, 'Biology',              'Chemistry', 20, 2),
('carol@example.com',   '$2b$10$R97gWO76R5TgXjRhl6jji.kxSRC0K8PhBkLAOfMfskAc1AMZHBz/2', 'Carol', 'Clark',    NULL, 'Mathematics',          NULL,        30, 3),
('dave@example.com',    '$2b$10$R97gWO76R5TgXjRhl6jji.kxSRC0K8PhBkLAOfMfskAc1AMZHBz/2', 'Dave',  'Dawson',   NULL, 'Physics',              NULL,        40, 4),
('eve@example.com',     '$2b$10$R97gWO76R5TgXjRhl6jji.kxSRC0K8PhBkLAOfMfskAc1AMZHBz/2', 'Eve',   'Evans',    NULL, 'Chemistry',            'Biology',   50, 5);

-- 2) roles
INSERT INTO roles (name, description) VALUES
('Admin',     'Administrator role with full access'),
('Student',   'Standard student role'),
('TA',        'Teaching assistant role'),
('Professor', 'Professor role');

-- 3) permissions — CRUD por entidad
INSERT INTO permissions (name, description) VALUES
('Create permissions', 'Can create permissions'),
('Read permissions', 'Can read permissions'),
('Update permissions', 'Can update permissions'),
('Delete permissions', 'Can delete permissions'),
('Create roles_permissions', 'Can create roles_permissions'),
('Read roles_permissions', 'Can read roles_permissions'),
('Update roles_permissions', 'Can update roles_permissions'),
('Delete roles_permissions', 'Can delete roles_permissions'),
('Create roles', 'Can create roles'),
('Read roles', 'Can read roles'),
('Update roles', 'Can update roles'),
('Delete roles', 'Can delete roles'),
('Create user_roles', 'Can create user_roles'),
('Read user_roles', 'Can read user_roles'),
('Update user_roles', 'Can update user_roles'),
('Delete user_roles', 'Can delete user_roles'),
('Create users', 'Can create users'),
('Read users', 'Can read users'),
('Update users', 'Can update users'),
('Delete users', 'Can delete users'),
('Create attendance_supp_sessions', 'Can create attendance_supp_sessions'),
('Read attendance_supp_sessions', 'Can read attendance_supp_sessions'),
('Update attendance_supp_sessions', 'Can update attendance_supp_sessions'),
('Delete attendance_supp_sessions', 'Can delete attendance_supp_sessions'),
('Create supplementary_sessions', 'Can create supplementary_sessions'),
('Read supplementary_sessions', 'Can read supplementary_sessions'),
('Update supplementary_sessions', 'Can update supplementary_sessions'),
('Delete supplementary_sessions', 'Can delete supplementary_sessions'),
('Create user_courses', 'Can create user_courses'),
('Read user_courses', 'Can read user_courses'),
('Update user_courses', 'Can update user_courses'),
('Delete user_courses', 'Can delete user_courses'),
('Create courses', 'Can create courses'),
('Read courses', 'Can read courses'),
('Update courses', 'Can update courses'),
('Delete courses', 'Can delete courses'),
('Create user_badges', 'Can create user_badges'),
('Read user_badges', 'Can read user_badges'),
('Update user_badges', 'Can update user_badges'),
('Delete user_badges', 'Can delete user_badges'),
('Create experience_badges', 'Can create experience_badges'),
('Read experience_badges', 'Can read experience_badges'),
('Update experience_badges', 'Can update experience_badges'),
('Delete experience_badges', 'Can delete experience_badges'),
('Create posts', 'Can create posts'),
('Read posts', 'Can read posts'),
('Update posts', 'Can update posts'),
('Delete posts', 'Can delete posts'),
('Create replies', 'Can create replies'),
('Read replies', 'Can read replies'),
('Update replies', 'Can update replies'),
('Delete replies', 'Can delete replies');

-- 4) roles_permissions
--    Admin → CRUD de todas las entidades
--    TA/Professor → Create, Read, Update supplementary_sessions y attendance_supp_sessions
--    Todos los roles → CRUD en posts y replies
INSERT INTO roles_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON r.name = 'Admin'
WHERE p.name IN (
	'Create permissions',
	'Read permissions',
	'Update permissions',
	'Delete permissions',
	'Create roles_permissions',
	'Read roles_permissions',
	'Update roles_permissions',
	'Delete roles_permissions',
	'Create roles',
	'Read roles',
	'Update roles',
	'Delete roles',
	'Create user_roles',
	'Read user_roles',
	'Update user_roles',
	'Delete user_roles',
	'Create users',
	'Read users',
	'Update users',
	'Delete users',
	'Create attendance_supp_sessions',
	'Read attendance_supp_sessions',
	'Update attendance_supp_sessions',
	'Delete attendance_supp_sessions',
	'Create supplementary_sessions',
	'Read supplementary_sessions',
	'Update supplementary_sessions',
	'Delete supplementary_sessions',
	'Create user_courses',
	'Read user_courses',
	'Update user_courses',
	'Delete user_courses',
	'Create courses',
	'Read courses',
	'Update courses',
	'Delete courses',
	'Create user_badges',
	'Read user_badges',
	'Update user_badges',
	'Delete user_badges',
	'Create experience_badges',
	'Read experience_badges',
	'Update experience_badges',
	'Delete experience_badges',
	'Create posts',
	'Read posts',
	'Update posts',
	'Delete posts',
	'Create replies',
	'Read replies',
	'Update replies',
	'Delete replies'
);

INSERT INTO roles_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON r.name IN ('TA', 'Professor')
WHERE p.name IN (
	'Create attendance_supp_sessions',
	'Read attendance_supp_sessions',
	'Update attendance_supp_sessions',
	'Create supplementary_sessions',
	'Read supplementary_sessions',
	'Update supplementary_sessions'
);

INSERT INTO roles_permissions (role_id, permission_id)
SELECT r.id, p.id
FROM roles r
JOIN permissions p ON r.name IN ('Admin', 'Student', 'TA', 'Professor')
WHERE p.name IN (
	'Create posts',
	'Read posts',
	'Update posts',
	'Delete posts',
	'Create replies',
	'Read replies',
	'Update replies',
	'Delete replies'
)
AND NOT EXISTS (
	SELECT 1
	FROM roles_permissions rp
	WHERE rp.role_id = r.id AND rp.permission_id = p.id
);

-- 5) courses
INSERT INTO courses (name, credits, duration, start_date) VALUES
('Intro SE',    3, 12, '2026-02-01 09:00:00'),
('Biology 101', 4, 14, '2026-03-01 09:00:00'),
('Calculus I',  5, 16, '2026-04-01 09:00:00'),
('Physics I',   4, 12, '2026-05-01 09:00:00'),
('Chemistry I', 3, 10, '2026-06-01 09:00:00');

-- 6) users_courses
INSERT INTO users_courses (user_id, course_id, relation_type) VALUES
(1, 1, 'student'),
(2, 2, 'student'),
(3, 3, 'student'),
(4, 4, 'ta'),
(5, 5, 'student');

-- 7) users_roles
--    alice, bob, carol → Student
--    dave → Admin
--    eve  → Professor
--    bob  → TA (ademas de Student)
INSERT INTO users_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'Student'
WHERE u.email IN ('alice@example.com', 'bob@example.com', 'carol@example.com');

INSERT INTO users_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'Admin'
WHERE u.email = 'dave@example.com';

INSERT INTO users_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'Professor'
WHERE u.email = 'eve@example.com';

INSERT INTO users_roles (user_id, role_id)
SELECT u.id, r.id
FROM users u
JOIN roles r ON r.name = 'TA'
WHERE u.email = 'bob@example.com';

-- 8) experience_badges
INSERT INTO experience_badges (name, min_level, message, associate_prices) VALUES
('Novice',       1,  'Welcome rookie', 'Sticker Pack'),
('Learner',      3,  'Keep going',     'Digital Certificate'),
('Intermediate', 6,  'Nice progress',  'Exclusive Avatar Frame'),
('Advanced',     10, 'Great work',     'Special Badge Highlight'),
('Expert',       20, 'Top performer',  'Access to VIP Forum');

-- 9) users_badges
INSERT INTO users_badges (user_id, experience_badge_id, date_acquired) VALUES
(1, 1, '2026-01-10 10:00:00'),
(2, 2, '2026-02-11 11:00:00'),
(3, 3, '2026-03-12 12:00:00');

-- 10) supplementary_sessions
INSERT INTO supplementary_sessions (requested_date, completed, topic, virtual) VALUES
('2026-02-20 09:00:00', false, 'Extra help 1', true),
('2026-03-21 10:00:00', true,  'Extra help 2', false),
('2026-04-22 11:00:00', false, 'Extra help 3', true);

-- 11) attendance_supp_sessions
--     dave (id 4) es el TA en todas
INSERT INTO attendance_supp_sessions (ta_id, student_id, supp_session_id, attendance_notes, additional_homework) VALUES
(4, 1, 1, 'Attended ok',   NULL),
(4, 2, 2, 'Attended late', NULL),
(4, 3, 3, 'Missed parts',  'Read chapter 1');

-- 12) posts
INSERT INTO posts (user_id, title, question, date_added) VALUES
(1, 'Help with HW',     'How do I solve problem 1?',    '2026-02-01 09:00:00'),
(2, 'Project question', 'How to structure my project?', '2026-03-01 10:00:00'),
(3, 'Exam tips',        'Best way to prepare?',         '2026-04-01 11:00:00');

-- 13) replies
INSERT INTO replies (post_id, user_id, reply_id, reply_message, date_added, likes, approvals) VALUES
(1, 2, NULL, 'Try starting with definitions', '2026-02-01 10:00:00', 0, 0),
(1, 3, 1,    'Also check examples',           '2026-02-01 11:00:00', 1, 0),
(2, 1, NULL, 'I used this guide',             '2026-03-01 11:30:00', 2, 1);

COMMIT;

