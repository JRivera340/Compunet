...
describe('Roles E2E (CRUD /role + /user-role)', () => {
    ...
    describe('POST /role', () => {
        ...
        it('should return 401 when no token is provided', async () => {
            await request(app.getHttpServer()).post('/role').send({ name: 'Professor' }).expect(401);
        });
    });
    ...
    describe('GET /role', () => {
        ...
        it('should return 403 when Student tries to list roles', async () => {
            await request(app.getHttpServer()).get('/role').set('Authorization', `Bearer ${studentToken}`).expect(403);
        });
